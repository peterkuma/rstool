...
- [InterMet Systems](https://www.intermetsystems.com) (iMet) radiosondes such
  as iMet-1-ABxn, data files produced by the iMetOS-II software (`.dat`).
...
- `imet`: InterMet Systems iMet-1-ABxn. `input` should be a directory
  generated by the iMetOS-II software.
- `prof`: rstool profile (`prof`) dataset (NetCDF).
- `pts`: rstool points (`pts`) dataset (NetCDF). Collection of measurement
  points. The output of running rstool with the output type `pts`.
- `raw:<instrument>`: "Raw" instrument-dependent rstool format (NetCDF).
  `instrument` is one of `imet`, `ws`.
- `ws` - Windsond. `input` should be a `.sounding` file generated by the
  Windsond software.
...
- `raw`: "Raw" instrument-dependent rstool format (NetCDF).
...
- `prof prof`: Profile to profile. This can be used to calculate derived
  physical quantities from a basic set of quantities.
...
ascending profile (default), only strictly increasing subset of points is
...
NetCDF by reading an input `.dat` file.
...
NetCDF by reading an input `.sounding` file.
...
rstool writes the following attributes in the raw (`raw:`*instrument*), points
(`pts`) and profile (`prof`) NetCDF files.
